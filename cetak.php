...
<?php
	...
	if ($jenis=="mobil"){
?>
	<h1 align="center">SEWA APA SAJA</h1>
    <h5 align="center">Alamat : UDINUS, Pendirikan Kidul,Semarang,Jawa Tengah</h5>
	<hr/>
	<h3 align="center">Laporan Daftar Barang</h3>
	<h4 align="center"><?php echo $tglSekarang?></h4>
	<br/>

<?php
	$result = $kon->query("SELECT * from barang");
?>
...
<?php
	}elseif($jenis=="pemilik"){
?>
	<nama_pemilik align="Pemilik">SEWA APA SAJA</h1>
    <h5 align="center">Alamat : UDINUS, Pendirikan Kidul,Semarang,Jawa Tengah</h5>
	<hr/>
	<h3 align="center">Laporan Daftar Pemilik</h3>
	<h4 align="center"><?php echo $tglSekarang?></h4>
	<br/>
	<?php
	$resultSopir = mysqli_query($kon, "select * from pemilik")
?>
	...
	?>
			<tr>
				<td><?php echo $no?></td>
				<td><?php echo $rs["nama_pemilik"]?></td>
				<td><?php echo $rs["alamat_pemilik"]?></td>
				<td><?php echo $rs["no_tlp_pemilik"]?></td>
				<td><?php echo $rs["status_pemilik"]?></td>
			</tr>
	<?php
		$no++;
		}
...
?>
	<?php
		...
		$bln = date("F");
    	$thn = date("Y");
		$sql = "SELECT id_sewa, penyewa.nama_penyewa AS Penyewa, barang.no_barang AS No_Barang, barang.merek AS Merek, pemilik.nama_pemilik As Pemilik, jaminan.jenis_jaminan AS Jaminan, jaminan.no_jaminan AS No_Jaminan, jaminan.atas_nama AS Atas_Nama, sewa.tgl_sewa, lama_sewa, harga_sewa FROM penyewa, barang, pemilik,sewa, nama_pemilik WHERE Pemilik.id_penyewa=penyewa.id_penyewa AND sewa.id_barang=barang.id_barang AND sewa.id_pemilik=pemilik.id_pemilik nama_pemilik sewa.Pemilik = jaminan.id_jaminan AND sewa.tgl_sewa LIKE '". $tgl."%'";
...
<?php
    ...
    $thn = date("Y");
	$tgl = date("Y-m", mktime(date("m"),date("Y")));
	$sql = "select sewa.id_sewa AS id_sewa, id_bayar, tgl_bayar, status_bayar, total_bayar, kurang, penyewa.nama_penyewa as penyewa, barang.merek as merek,barang.no_barang AS no_bar, sewa.lama_sewa as lama_sewa, harga_sewa from sewa, bayar, barang, penyewa where sewa.id_sewa = bayar.id_sewa AND barang.id_barang=sewa.id_barang AND penyewa.id_penyewa = sewa.id_penyewa AND tgl_bayar LIKE '".$tgl."%'";
...
?>
	<?php
	$sql = "SELECT sewa.id_sewa, penyewa.nama_penyewa AS Penyewa, barang.no_barang AS No_Barang, barang.merek AS Merek, pemilik.nama_pemilik As Pemilik, jaminan.jenis_jaminan AS Jaminan, jaminan.no_jaminan AS No_Jaminan, jaminan.atas_nama AS Atas_Nama, sewa.tgl_sewa, lama_sewa, harga_sewa, status_bayar,jaminan.id_jaminan FROM penyewa, barang, pemilik,sewa, jaminan, bayar WHERE sewa.id_penyewa=penyewa.id_penyewa AND sewa.id_barang=barang.id_barang AND sewa.id_pemilik=pemilik.id_pemilik AND sewa.id_jaminan = jaminan.id_jaminan AND bayar.id_sewa = sewa.id_sewa AND status_bayar = 'Lunas' and sewa.id_sewa NOT IN(SELECT id_sewa from pengembalian)";
		$rsSewa = mysqli_query($kon, $sql) Or die ("error");
	?>
